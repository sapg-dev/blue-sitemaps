...
const LinkCard = ({ title, content, link, website, date, type, id, daysAgo, state }) => {
    ...
    const handleActionButtonClick = async (actionType, e, articleId) => {
      ...
      handleMenuClose();
      try {
        const response = await axios.patch(`http://localhost:5000/update-article/${articleId}`, { newState: actionType });
  ...
  return (
    <Card sx={cardStyle}>
      ...
      >
        ...
        <MenuItem onClick={(e) => handleActionButtonClick('Reading List', e, id)}>Reading List</MenuItem>
